...
const categoryName = computed(() => {
    ...
    // đảm bảo luôn trả về string (tránh trường hợp mảng)
...
async function fetchPublicQuizzes(page = 0) {
    ...
    error.value = ''
    try {
        const res = await api.get(`/quiz/public/category/${categoryId.value}`, {
        ...
        const transformed = list.map(q => ({
            quiz_id: q.id,
            title: q.title,
            fullName: q.user?.fullName || q.user?.username || 'Unknown',
            ownerId: q.user?.id ?? q.userId ?? null,
        ...
        const pageIdx = Number.isFinite(res.data.currentPage) ? res.data.currentPage : page
        const total = Number.isFinite(res.data.totalPages) ? res.data.totalPages : 1

        quizCache.set(pageIdx, {
            content: transformed,
            page: pageIdx,
            totalPages: total,
        })
...
async function playQuiz(quizId) {
    ...
    try {
        ...
        router.push({ name: 'PlayQuiz', params: { quizId: resp.attemptId } })
    } catch (e) {
...
const reportQuiz = async (quiz) => {
    // đảm bảo đã có userId (nếu chưa, thử khôi phục từ token/localStorage)
    if (!userId.value) {
        try { await getUserId() } catch (e) {/* bỏ qua nếu không cần bắt buộc đăng nhập để xem danh sách */ }
    }

    // Nếu là quiz của chính người dùng -> show thông báo và return
    if (userId.value && quiz.ownerId && Number(userId.value) === Number(quiz.ownerId)) {
        // Cách 1: alert đơn giản
        alert('Không thể báo cáo quiz của bạn');
        // Cách 2 (tuỳ bạn): hiển thị toast UI đẹp hơn
        // showToast('Không thể báo cáo quiz của bạn', 'warning')
        return;
    }
    ...
    const quizData = {
        ...
        ownerId: quiz.ownerId, // truyền sang modal (để BE/FE chặn thêm nếu cần)
    }
...
watch(() => route.params.id, (val) => {
    ...
    categoryId.value = idNum
    quizCache.clear()
    fetchPublicQuizzes(0)
})
                        ...
                        <div class="quiz-content">
                            <div class="quiz-header">
                                ...
                                <div class="quiz-actions">
                                    <button @click="reportQuiz(quiz)" class="action-btn report-btn"
                                        title="Báo cáo quiz">
